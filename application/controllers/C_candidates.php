  ...
  // end function add family income resource
   ...
   // function add loan & Debts
    ...
    $lastIds =  $this->m_can->getLastId();
		...
		$result['familyLoan'] = $this->m_can->addLoan($amount,$institution,$interest,$reason,$monthly,$trimester,$semester,$capital,$lastCanId);
		echo json_encode($msg);
	}
   // End funciton add loan & Debts
    ...
    $lastIds =  $this->m_can->getLastId();
		...
		$result['familyExpense'] = $this->m_can->addExpense($rice,$food,$firewood,$loan,$study,$medical,$electric,$agriculture,$wedding,$other,$totalExpense,$lastCanId);
    ...
    $lastIds =  $this->m_can->getLastId();
		...
		$result['formExpense'] = $this->m_can->addResidence($status,$age,$rating,$lastCanId);
		echo json_encode($msg);
	}
    ...
    public function addConclude()
    {
      ...
      $lastIds =  $this->m_can->getLastId();
    	...
    	$data = $this->m_can->addConclude($investigatorConclude,$lastCanId);
      $msg['success'] = false;
      $msg['type'] = 'add';
